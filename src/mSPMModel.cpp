...
static inline void v9_copy (float d[9], const float s[9]) { for(int i=0;i<9;i++) d[i]=s[i]; }
static inline void v9_zero (float x[9])                   { for(int i=0;i<9;i++) x[i]=0.f; }
static inline void v9_add  (float o[9], const float a[9], const float b[9]) { for(int i=0;i<9;i++) o[i]=a[i]+b[i]; }
static inline void v9_sub  (float o[9], const float a[9], const float b[9]) { for(int i=0;i<9;i++) o[i]=a[i]-b[i]; }
static inline void v9_scale(float o[9], const float a[9], float s)          { for(int i=0;i<9;i++) o[i]=s*a[i]; }

static inline float v9_dot   (const float a[9], const float b[9]) { float s=0.f; for(int i=0;i<9;i++) s+=a[i]*b[i]; return s; }
...
static inline void m99_zero(float A[9][9]) {
    for(int r=0;r<9;r++) for(int c=0;c<9;c++) A[r][c]=0.f;
}
static inline void m99_mul_v9(float y[9], const float A[9][9], const float x[9]) {
    for(int r=0;r<9;r++) { float s=0.f; for(int c=0;c<9;c++) s+=A[r][c]*x[c]; y[r]=s; }
}
...
static void fk_residual_and_jac(const FKProblem& P, const float x[9],
                                  float F[9], float J[9][9])
{
    // extract w vectors
    ...
    for(int i=0;i<3;i++) for(int k=0;k<3;k++) J[i][3*i+k] = P.v[i][k];

    // row 3: F3 = w1·w2  →  ∂/∂w1 = w2,  ∂/∂w2 = w1
    for(int k=0;k<3;k++) { J[3][3+k] = w2[k]; J[3][6+k] = w1[k]; }

    // row 4: F4 = w2·w0  →  ∂/∂w2 = w0,  ∂/∂w0 = w2
    for(int k=0;k<3;k++) { J[4][6+k] = w0[k]; J[4][0+k] = w2[k]; }

    // row 5: F5 = w0·w1  →  ∂/∂w0 = w1,  ∂/∂w1 = w0
    for(int k=0;k<3;k++) { J[5][0+k] = w1[k]; J[5][3+k] = w0[k]; }

    // rows 6-8: ∂(w_i·w_i)/∂w_i = 2·w_i  (power rule)
    for(int i=0;i<3;i++) for(int k=0;k<3;k++) J[6+i][3*i+k] = 2.f*x[3*i+k];
}

// ── normal equations: g = J^T F,  B = J^T J ──────────────────────────────────

static void fk_normal_eqs(const float F[9], const float J[9][9],
                           float g[9], float B[9][9])
{
    v9_zero(g);
    m99_zero(B);
    for(int r=0;r<9;r++)
        for(int c=0;c<9;c++)
            g[c] += J[r][c]*F[r];            // g = J^T F
...
static bool chol9(const float A[9][9], float L[9][9])
{
    ...
    for(int i=0;i<9;i++) {
        for(int j=0;j<=i;j++) {
            ...
            if(i==j) { if(s<=1e-12f) return false; L[i][j]=sqrtf(s); }
            else       L[i][j] = s/L[j][j];
        }
    }
    return true;
...
static bool dogleg_step(const float g[9], const float B[9][9], float delta, float p[9])
{
    ...
    if(v9_norm(pB) <= delta) { v9_copy(p,pB); return true; }  // inside trust region

    // Cauchy step pU: optimal step along -g
    float Bg[9]; m99_mul_v9(Bg,B,g);
    const float gg=v9_dot(g,g), gBg=v9_dot(g,Bg);
    float pU[9];
    if(gBg <= 1e-20f) { v9_scale(pU,g,-1.f); }
    else               { v9_scale(pU,g,-gg/gBg); }

    const float nU=v9_norm(pU);
    if(nU >= delta) {                                          // Cauchy step outside
        v9_scale(p,pU,delta/nU); return true;
    }

    // Interpolate pU → pB to hit trust-region boundary
    float d[9]; v9_sub(d,pB,pU);
    const float a=v9_normsq(d),
                b=2.f*v9_dot(pU,d),
                c=v9_normsq(pU)-delta*delta;
...
static FKSolverOut fk_solve(const FKProblem& P, const float x0[9])
{
    ...
    for(int iter=0; iter<100; ++iter) {
        ...
        if(v9_norm(g) < gtol) { out.success=true; break; }

        float p[9];
        if(!dogleg_step(g,B,delta,p)) break;

        const float pn=v9_norm(p);
        if(pn < xtol) { out.success=true; break; }

        const float pred=pred_reduction(g,B,p);
        if(pred <= 1e-20f) break;

        float xt[9]; v9_add(xt,out.x,p);
        float Ft[9], Jt[9][9]; fk_residual_and_jac(P,xt,Ft,Jt);
        const float cost_t=0.5f*v9_dot(Ft,Ft);
        const float rho=(out.cost-cost_t)/pred;

        // trust-region radius update
        if(rho < 0.25f)                                delta *= 0.25f;
        else if(rho > 0.75f && fabsf(pn-delta)<1e-5f) delta  = fminf(2.f*delta, delta_max);

        if(rho > eta) {
            v9_copy(out.x, xt);
            if(fabsf(out.cost-cost_t) < ftol) { out.success=true; out.cost=cost_t; break; }
        }
...
SPMArch::SPMArch()
{
    ...
    eta_i1[0] = 0.0f;      eta_i1[1] = two_pi_3;   eta_i1[2] = four_pi_3;
    eta_i2[0] = 0.0f;      eta_i2[1] = two_pi_3;   eta_i2[2] = four_pi_3;

    gamma_i1[0] = 0.0f;    gamma_i1[1] = 0.0f;     gamma_i1[2] = 0.0f;

    // pi/2 - 5 deg small tilt to avoid singularity at neutral pose
    const float mp_tilt = (float)M_PI / 2.0f - 0.08726646f;
    gamma_i2[0] = mp_tilt; gamma_i2[1] = mp_tilt;  gamma_i2[2] = mp_tilt;

    const float pi_3 = (float)M_PI / 3.0f;
    alpha_i1[0] = pi_3;    alpha_i1[1] = pi_3;     alpha_i1[2] = pi_3;

    const float pi_4 = (float)M_PI / 4.0f;
    alpha_i2[0] = pi_4;    alpha_i2[1] = pi_4;     alpha_i2[2] = pi_4;
}
...
mVector3f SPMModel::computeVVector(float theta,
                                   ...
                                   float a3, float c3)
{
    ...
    return mVector3f(a1 * ct + b1 * st + c1,
                     a2 * ct + b2 * st + c2,
                     a3 * ct           + c3);
}
...
SPMModel::SPMModel(const SPMArch& arch)
    : is_initialized(false), arch_(arch)
{
    ...
    bool all_valid = true;
    for (int i = 0; i < SPM_LEGS; i++) {
        if (!neutral.has_solution[i]) { all_valid = false; break; }
    }
...
IKResult SPMModel::computeIK(const Matrix33f& R, bool update_config)
{
    ...
    for (int i = 0; i < SPM_LEGS; i++) {
        ...
        float C = cosf(arch_.alpha_i2[i])
                - (wx * c1_i[i] + wy * c2_i[i] + wz * c3_i[i]);
    ...
    if (update_config) {
        bool all_valid = true;
        for (int i = 0; i < SPM_LEGS; i++) {
            if (!result.has_solution[i]) { all_valid = false; break; }
        }
        ...
        if (all_valid) {
            for (int i = 0; i < SPM_LEGS; i++) {
                ...
                theta_i[i] = th;
                w_i[i]     = w[i];
                v_i[i]     = computeVVector(th,
                                            a1_i[i], b1_i[i], c1_i[i],
                                            a2_i[i], b2_i[i], c2_i[i],
                                            a3_i[i], c3_i[i]);
...
VelJacobian SPMModel::computeVelJacobian() const
{
    ...
    for (int i = 0; i < SPM_LEGS; i++) {
        ...
        if (i == 0) {
            jac.A_mat.m11 = wxv.x; jac.A_mat.m12 = wxv.y; jac.A_mat.m13 = wxv.z;
            jac.B_mat.m11 = u_i[i] ^ wxv;  // dot product operator^
        } else if (i == 1) {
            jac.A_mat.m21 = wxv.x; jac.A_mat.m22 = wxv.y; jac.A_mat.m23 = wxv.z;
            jac.B_mat.m22 = u_i[i] ^ wxv;
        } else {
            jac.A_mat.m31 = wxv.x; jac.A_mat.m32 = wxv.y; jac.A_mat.m33 = wxv.z;
...
FKResult SPMModel::computeFK(const float theta[SPM_LEGS],
                              bool update_config,
                              bool init_prev_sol)
{
    FKResult result{};
    ...
    for (int i = 0; i < SPM_LEGS; i++) {
        mVector3f vi = computeVVector(theta[i],
                                      a1_i[i], b1_i[i], c1_i[i],
                                      a2_i[i], b2_i[i], c2_i[i],
                                      a3_i[i], c3_i[i]);
        P.v[i][0] = vi.x;
